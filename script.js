...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Função para definir o progresso sem animação
    ...
    // Obter o valor do input e definir o progresso automaticamente
    const inputValue = parseInt(document.getElementById('progressInput').value);
    const percentage = Math.min(Math.max(inputValue, 0), 100); // Garantir que esteja entre 0 e 100
    setProgress(percentage); // Define o progresso diretamente ao carregar a página
});
